...
- Đường dẫn script cài đặt: [install-metrics-server.sh.example](file:///d:/Code/Deploy/templates/shared/metrics-server/install-metrics-server.sh.example)
...
> [!NOTE]
> Để tìm hiểu chi tiết về cài đặt thủ công và cấu hình tùy chỉnh nâng cao của Metrics Server, vui lòng tham khảo tài liệu: [Metrics Server README](file:///d:/Code/Deploy/templates/shared/metrics-server/README.md).
...
- Đường dẫn HPA mẫu: [hpa.yml.example](file:///d:/Code/Deploy/templates/kubernetes/hpa/hpa.yml.example)
...
> - Chi tiết các lệnh quản lý HPA nâng cao (`kubectl get`, `describe`, `delete`) và cơ chế chống dao động Pod (flapping) được hướng dẫn cụ thể tại: [HPA README](file:///d:/Code/Deploy/templates/kubernetes/hpa/README.md).
