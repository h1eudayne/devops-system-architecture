...
| Tiêu chí | Relational Database (SQL) | Non Relational Database (NoSQL) |
|---|---|---|
| **Cấu trúc dữ liệu** | Bảng, Hàng, Cột cần định nghĩa rõ ràng trước khi có thể ghi data. | Không cần định nghĩa trước cấu trúc dữ liệu. |
| **Tính linh hoạt** | Cấu trúc của bảng cần được tuân thủ, kém linh hoạt hơn. | Linh hoạt cao. Dễ dàng thêm thuộc tính. |
| **Hiệu suất** | Tối ưu cho các thao tác truy vấn phức tạp, join bảng. VD Thống kê data, analytic. | Tối ưu cho xử lý tốc độ cao. |
| **Bảo mật** | Bảo mật cao do có cấu trúc dữ liệu rõ ràng. | Khó khăn trong việc quản lý và bảo mật truy cập. |
| **Tính mở rộng** | Khả năng mở rộng kém, thường phải thay đổi cấu hình database. | Khả năng mở rộng dễ dàng. Scale horizontal. |
