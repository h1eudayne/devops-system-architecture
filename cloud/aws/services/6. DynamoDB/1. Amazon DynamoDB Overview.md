...
|---|---|---|
| **Mô hình dữ liệu** | Dạng bảng gồm các Hàng (Rows) và Cột (Columns) cố định. | Đa dạng: Key-Value, Document, Column, Graph. |
| **Tính chất Schema** | **Cứng nhắc (Rigid Schema)**. Phải khai báo schema trước khi chèn dữ liệu. | **Linh hoạt (Dynamic/Schemaless)**. Dữ liệu tự định nghĩa cấu trúc của chính nó. |
| **Quan hệ dữ liệu** | Hỗ trợ các quan hệ phức tạp thông qua Khóa ngoại (Foreign Keys) và lệnh `JOIN`. | Thường **không hỗ trợ JOIN**. Dữ liệu liên quan được thiết kế gộp (Denormalized) trong một bản ghi. |
| **Khả năng mở rộng (Scaling)** | **Scale Vertical** (Nâng cấp CPU, RAM trên một máy chủ). | **Scale Horizontal** (Thêm nhiều máy chủ giá rẻ vào cụm phân tán). |
| **Tính nhất quán** | Đảm bảo tính nhất quán dữ liệu tuyệt đối theo chuẩn **ACID** (Atomicity, Consistency, Isolation, Durability). | Thường ưu tiên tính sẵn sàng cao và chấp nhận **Eventual Consistency** (nhất quán sau một khoảng thời gian ngắn). |
| **Ngôn ngữ truy vấn** | Sử dụng ngôn ngữ chuẩn **SQL** (Structured Query Language). | Sử dụng các API riêng hoặc ngôn ngữ truy vấn riêng của từng DB. |
