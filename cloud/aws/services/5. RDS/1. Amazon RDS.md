...
## VI. Amazon Aurora (Giải pháp Database thế hệ mới)
...
### 2. Hai hình thức triển khai chính của Amazon Aurora
* **Aurora Provisioned Cluster (Master + Multi Read Replica)**: Cấu hình tài nguyên phần cứng cố định cho cụm máy chủ. Gồm 1 Primary DB Instance (phục vụ cả Đọc/Ghi) và nhiều bản sao chỉ đọc (Read Replicas - hỗ trợ tối đa lên đến 15 bản sao) chia sẻ chung một lớp lưu trữ ảo hóa dùng chung (shared storage cluster volume).
* **Aurora Serverless**: Tự động hóa việc khởi tạo, mở rộng dung lượng và tắt bớt tài nguyên máy chủ dựa trên lưu lượng thực tế của ứng dụng. Rất hữu ích cho các tải công việc biến động khó dự đoán trước hoặc chạy không thường xuyên để tối ưu hóa chi phí.

### 3. Những tính năng vượt trội của Aurora so với RDS thông thường
* **Hiệu năng vượt trội hơn**: Tốc độ xử lý nhanh hơn đáng kể (theo quảng cáo của AWS, Aurora cho hiệu năng đọc/ghi gấp 5 lần đối với MySQL và gấp 3 lần đối với PostgreSQL so với việc triển khai các engine này trên RDS instance thông thường có cùng thông số phần cứng).
* **Hỗ trợ tính năng Backtracking (Quay ngược thời gian)**: 
  * Cho phép người quản trị "quay ngược thời gian" và đưa cơ sở dữ liệu về một thời điểm chính xác trong quá khứ (lên tới tối đa **72 giờ** trước đó) chỉ trong vài giây.
  * **Sự khác biệt lớn so với Restore từ Snapshot**: Khôi phục từ snapshot đòi hỏi AWS phải khởi tạo một database instance mới hoàn toàn từ đầu (rất tốn thời gian đối với dữ liệu lớn và phải đổi endpoint). Trong khi đó, **Backtracking** khôi phục trực tiếp ngay trên chính Cluster đang chạy giúp giảm tối thiểu thời gian khôi phục dịch vụ (RTO).
* **Tự động quản lý Endpoint cấp độ Cluster**:
  * Tự động cung cấp và cập nhật trạng thái của hai Endpoint cố định mức cụm: **Writer Endpoint** (định tuyến tự động tới máy chủ ghi hiện hành) và **Reader Endpoint** (tự động chia đều tải đọc qua cơ chế cân bằng tải đến tất cả các bản sao Read Replicas). Người dùng không bao giờ cần phải chỉnh sửa cấu hình ứng dụng khi xảy ra sự cố hoán đổi node (Failover).

---

## VII. Các trường hợp sử dụng (Use Cases) của Amazon RDS
...
## VIII. Tài liệu thực hành liên quan (Hands-on Labs)

* **[Amazon RDS Hands-on Lab (Basic)](2.%20Amazon%20RDS%20Hands-on%20Lab%28Basic%29.md)**: Hướng dẫn tạo một RDS Instance đơn lẻ với engine MySQL và thực hành kết nối DBeaver, CRUD & giả lập Slow Query.
* **[Amazon RDS Hands-on Lab (Cluster)](3.%20Amazon%20RDS%20Hands-on%20Lab%28Cluster%29.md)**: Hướng dẫn khởi tạo cụm cơ sở dữ liệu Multi-AZ DB Cluster (1 Writer, 2 Readers), cơ cấu hoạt động của các Node, các loại Endpoint và kết nối ứng dụng bằng Cluster Writer Endpoint cố định.
