...
### Bước 3: Cấu hình mở quyền truy cập công khai (Public Access) cho Website
1. Chuyển sang tab **Permissions** (Quyền truy cập) của Bucket.
2. Tại mục đầu tiên **Block public access (bucket settings)** (hiện tại đang là *On*), nhấp chọn nút **Edit**.

![Truy cập tab Permissions để chỉnh sửa Block public access](../../../../images/aws/s3_static_web_step3_permissions.png)

3. Bỏ tích chọn ở mục **Block all public access** (Chặn tất cả quyền truy cập công khai).
4. Nhấp nút **Save changes**. Nhập từ khóa `confirm` vào hộp thoại xuất hiện để xác nhận thay đổi.
5. **Thiết lập chính sách Bucket Policy**: Việc bỏ block public access mới chỉ là điều kiện cần. Để thực sự cho phép đọc đối tượng công khai, bạn bắt buộc phải cấu hình chính sách bucket policy.
   * Cuộn xuống mục **Bucket policy**, chọn **Edit** và dán đoạn mã chính sách JSON sau:
     ```json
     {
         "Version": "2012-10-17",
         "Statement": [
     ...
     ```
     *(Thay thế `h1eudayne` bằng tên S3 Bucket thực tế của bạn).*
6. Nhấp **Save changes** để áp dụng chính sách.
7. Thử tải lại liên kết **Bucket website endpoint** đã lấy ở Bước 2. Website tĩnh của bạn hiện tại đã hoạt động công khai và hiển thị đầy đủ giao diện trên trình duyệt web!
